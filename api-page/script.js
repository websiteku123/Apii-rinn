...
const fullDescription = "Rin apii adalah  platfrom rest api yang simple, fast respon, dan juga aman, Silahkan di coba :)";
        ...
        if (dynamicImage) {
            ...
            // FIX: Menggeser posisi banner ke kanan sebanyak 2% secara presisi memakai transform
        ...
        // FIX FIX-SPAM: Gunakan event delegation tunggal pada `apiContent` untuk handle klik tombol GET
        if (apiContent) {
            apiContent.addEventListener('click', event => {
                ...
                if (modalRefs.submitBtn) {
                    ...
                    // FIX SPAM: Bersihkan handler onclick lama agar tidak menumpuk saat modal dibuka berulang kali
                ...
                if (hasParams || isApikey) {
                    ...
                    modalRefs.submitBtn.onclick = async (e) => {
                        e.preventDefault(); // Mencegah double trigger bawaan form submit
